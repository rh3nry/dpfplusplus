...
union block
{
 public:
  ...
  inline bool operator[] (const size_t pos) const { return bits[pos]; }
  constexpr inline size_t size() const { return sizeof(__mX) * 8; }
  inline unsigned parity() const { return bits.count() % 2; }
//   inline void shiftr(const size_t pos) { bits >>= pos; }
//   inline void shiftl(const size_t pos) { bits <<= pos; }
  inline block shiftr(size_t pos) const { auto ret = bits; return ret >>= pos; }
  inline block shiftl(size_t pos) const { auto ret = bits; return ret <<= pos; }
  inline auto get_limb(size_t i) const { return mX[i]; }
